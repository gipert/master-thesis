...
\titlegraphic{\vspace{4.8cm}\begin{flushright}\includegraphics[width=1.8cm]{img/infn-logo.pdf}\hspace{5mm}\includegraphics[width=1.8cm]{../img/logo.pdf}\end{flushright}}
\date{18 Luglio 2017}
...
%	\end{exampleblock}
	\begin{itemize}
		\item {\gerda} misura la vita media del decadimento doppio-beta senza neutrini (\alert{$0\nbb$}) dell'isotopo \alert{\ce{^76Ge}}
		\item Array di rivelatori arricchiti al \ce{^76Ge} $\longmapsto$ \alert{rivelatore = sorgente}
		\item \alert{Fase II} --- Aumento della quantità di \ce{^76Ge} e aggiornamento della strumentazione per una maggiore efficienza di reiezione degli eventi di fondo.
	\end{itemize}
	\end{columns}
\end{frame}
\begin{frame}[plain, label=2]{The \alert{GER}manium \alert{D}etector \alert{A}rray}
	\begin{tikzpicture}[fill=white]
	\node at (0,0) {\includegraphics[width=\columnwidth]{../img/GERDAmodel.jpg}}; % CHE IMMAGINE DI MERDA
...
\begin{frame}[label=5]{\textit{Lorentz-violating} \texorpdfstring{2$\nbb$}{2nbb} per l'isotopo \ce{^76Ge}}
	...
	%\begin{simpleblock}\alert{\textit{Lorentz-violating} $2\nbb$}\end{simpleblock}
		In decadimenti con neutrini la violazione della simmetria di Lorentz porta a una distorsione dello spettro in energia ($K=K_1+K_2$) dei due elettroni in uscita.
...
\centerline{%
	...
	\begin{groupplot}[	group style = { group size = 1 by 2,
		...
		\nextgroupplot
			...
			\node at (axis cs:5300,3.9) {\includegraphics[width=2cm]{../img/bege_base.pdf}};
		\nextgroupplot
			...
			\fill[green, opacity=0.5] (axis cs:2014,0) rectangle (axis cs:2064,4.7);
			\node at (axis cs:5500,3.4) {\includegraphics[width=1.3cm]{../img/coax_base.pdf}};
	\end{groupplot}
\end{tikzpicture}%
}
\end{frame}
...
\end{table}
\end{frame}
%\begin{frame}{\texorpdfstring{$\alpha$}{alpha}-model}
%	Decadimenti $\alpha$ nella sottocatena \ce{^226Ra} $\rightarrow$ \ce{^210Po} sono stati simulati attorno ai contatti p$^+$.
%\end{frame}
\end{document}
